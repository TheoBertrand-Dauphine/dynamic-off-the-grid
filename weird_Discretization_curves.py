...
class PiecewiseGeodesic_RS(torch.nn.Module):
    ...
    def forward(self, timestamps):
        ...
        for k in range(self.order-1):
            ...
            if mask_k.sum() > 0:
                output = odeint(self.RHS, torch.stack([self.points[:,k].unsqueeze(1), self.tangent_vectors[:,k].unsqueeze(1)], dim=2), t=(timestamps[mask_k] - self.discretization_times[k]) / (self.discretization_times[k+1] - self.discretization_times[k]), method = 'dopri5', options={'step_size': 0.1})
    ...
    def fit(self, y, operator, timestamps, n_epoch=1, lr=1e-2, regul=1e-2, constr=1., n_sample=4):
        '''
        ...
        for epoch in tqdm(range(n_epoch)):
            ...
            constraint_loss = ((odeint(self.RHS, torch.stack([self.points, self.tangent_vectors], dim=2), t=torch.tensor([0., 1.]), method = 'dopri5', options={'step_size': 0.1})[-1, :, :-1, 0] - self.points[:,1:]) ** 2).mean(dim=[-1,-2])
            ...
            with torch.no_grad():
                ...
                result_ode = odeint(self.RHS_optim, y0, t=torch.linspace(0, 1, 2), method = 'dopri5', options={'step_size': 0.1})[-1]
